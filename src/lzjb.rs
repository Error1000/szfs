...
pub fn lzjb_decompress(data: &mut impl Iterator<Item = u8>, output_length: usize) -> Result<Vec<u8>, ()> {
    ...
    while output_buf.len() < output_length {
        ...
        if copymap & (copymask as u8) != 0 {
            ...
            let mlen = usize::from(byte0 >> (8-MATCH_BITS)) + MATCH_MIN;
            let lookback_size = ((((byte0 as u16) << 8) | (byte1 as u16)) as usize) & OFFSET_MASK;
            if lookback_size > output_buf.len() {
                return Err(());
            }
            let mut lookback_pos = output_buf.len() - lookback_size;
            for _ in 0..mlen {
